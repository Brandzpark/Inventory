...
import { getAllSalesRepsApi } from "@/api/salesReps";
import { ISalesRep } from "@/typings/salesReps";

type Props = {};

export default function SalesRepsTable({}: Props) {
  const searchParams = useSearchParams();

  const page = searchParams.get("page") || 1;
  const search = searchParams.get("search") || "";
  const [data, setdata] = useState<ISalesRep[] | []>([]);
  ...
  async function fetchData() {
    setLoading(true);
    const { data } = await getAllSalesRepsApi({ page, search });
