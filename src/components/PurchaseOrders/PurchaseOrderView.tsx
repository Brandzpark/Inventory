                            ...
                            <div className="mt-7 grid lg:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <Input
                                        className="mt-2"
                                        disabled
                                        value={purchaseOrder ? format(purchaseOrder.orderDate, "Y-MM-dd") : ""}
                                    />
                                </div>
                                <div>
                                    ...
                                    <Input
                                        className="mt-2"
                                        disabled
                                        value={purchaseOrder ? format(purchaseOrder.requiredDate, "Y-MM-dd") : ""}
                                    />
                                </div>
                            </div>
                            <CardTitle className="bg-[#FAFAFA] py-2 px-3 rounded-md mt-10">
                            ...
                            <div>
                                <div className="mt-7 border rounded-md relative overflow-x-auto">
                                    <table className="table-auto w-full text-sm">
                                        ...
                                        <tbody>
                                            {purchaseOrder?.items?.map((row: IPurchaseOrderItem, index: number) => {
                                                const total = parseFloat(row?.rate) * parseFloat(row?.quantity)
                                                const discountAmount = row?.discount ? total * parseFloat(row?.discount) / 100 : 0
                                                ...
                                                return (
                                                    <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                                        ...
                                                        <td className="px-3 py-4 min-w-[8rem]">
                                                            <div className="text-slate-500" >{row?.remark}</div>
                                                        </td>
                                                        <td align="right" className="px-3 py-4 min-w-[8rem]">
                                                            <div className="text-slate-500" >{row?.quantity}</div>
                                                        </td>
                                                        <td align="right" className="px-3 py-4 min-w-[8rem]">
                                                            <div className="text-slate-500" >{row?.rate}</div>
                                                        </td>
                                                        <td align="right" className="px-3 py-4 min-w-[8rem]">
                                                            <div className="text-slate-500" >{row?.discount}</div>
                                                        </td>
                                                        <td align="right" className="px-3 py-4 min-w-[8rem]">
                                                            <div className=" text-slate-500" >{formatMoney(String(subTotal))}</div>
                                                        </td>
                                                    </tr>
                                                );
                                            })}
                                    ...
                                    <div className="border-t grid grid-cols-2 items-start gap-5 p-6" >
                                        <Textarea disabled rows={5} placeholder="Remark" value={purchaseOrder?.remark} />
                                        <div className="border rounded-md text-sm">
                                            <table className="w-full" >
                                                <tbody>
                                                    <tr className="border-b" >
                                                        <td className="px-6 py-3 font-light" >Subtotal</td>
                                                        <td align="right" className="px-6 py-3 font-light" >{formatMoney(String(purchaseOrder?.subTotal))}</td>
                                                    </tr>
                                                    <tr className="border-b" >
                                                        <td className="px-6 py-3 font-light" >Discount</td>
                                                        <td align="right" className="px-6 py-3 font-light" >{formatMoney(String(parseFloat(purchaseOrder ? purchaseOrder?.subTotal : "0") - parseFloat(purchaseOrder ? purchaseOrder?.total : "0")))}</td>
                                                    </tr>
                                                    <tr className="font-semibold" >
                                                        <td className="px-6 py-3" >Total</td>
                                                        <td align="right" className="px-6 py-3" >{formatMoney(String(purchaseOrder?.total))}</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
